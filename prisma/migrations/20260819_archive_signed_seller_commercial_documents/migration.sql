...
BEGIN
  ...
  archived_document := COALESCE(wf->'currentDocumentJson', '{}'::jsonb)
    || jsonb_build_object(
      'workflowArchive', jsonb_build_object(
        'status', workflow_status,
        'agreementNumber', COALESCE(wf->>'agreementNumber',''),
        'instrumentId', instrument_id,
        'instrumentType', instrument_type,
        'effectiveDate', COALESCE(wf->>'effectiveDate',''),
        'validFrom', COALESCE(wf->>'validFrom',''),
        'validTo', COALESCE(wf->>'validTo',''),
        'lockedAt', COALESCE(wf->>'lockedAt',''),
        'sentForSignatureAt', COALESCE(wf->>'sentForSignatureAt',''),
        'sellerSignedAt', COALESCE(wf->>'sellerSignedAt',''),
        'companySignedAt', COALESCE(wf->>'companySignedAt',''),
        'closedAt', COALESCE(wf->>'closedAt',''),
        'sellerSignatureStatus', COALESCE(wf->>'sellerSignatureStatus',''),
        'sellerSignatureImageUrl', COALESCE(wf->>'sellerSignatureImageUrl',''),
        'sellerSignatureProcessedUrl', COALESCE(wf->>'sellerSignatureProcessedUrl',''),
        'sellerSignedDocumentUrl', COALESCE(wf->>'sellerSignedDocumentUrl',''),
        'companySignatoryId', COALESCE(wf->>'companySignatoryId',''),
        'signatories', COALESCE(wf->'signatories','[]'::jsonb),
        'archivedAt', NOW()
      )
    );
  ...
  SET
    ...
    "documentSnapshot" = CASE
      WHEN jsonb_typeof(archived_document) = 'object' AND archived_document <> '{}'::jsonb THEN archived_document
