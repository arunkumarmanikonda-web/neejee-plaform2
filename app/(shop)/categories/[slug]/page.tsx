...
function normalizeCategory(summary: CategorySummary | null | undefined, fallbackSlug: string): CategorySummary {
  ...
  return {
    ...
    breadcrumb: Array.isArray(summary.breadcrumb) && summary.breadcrumb.length > 0 ? summary.breadcrumb : derived.breadcrumb || [titleFromSlug(summary.slug || fallbackSlug)],
    breadcrumbSlugs: Array.isArray(summary.breadcrumbSlugs) && summary.breadcrumbSlugs.length > 0 ? summary.breadcrumbSlugs : derived.breadcrumbSlugs || [summary.slug || fallbackSlug],
  };
}
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        let facetsData: FacetsResponse = { crafts: [], regions: [], materials: [], occasions: [], badges: [], priceRange: { minPaise: 0, maxPaise: 0 }, total: 0 };
        ...
        const nextProducts = Array.isArray(productsData?.products) ? productsData.products.map(mapProductToCardData).filter((product) => product.id && product.slug) : [];
    ...
    <>
      ...
      <section className="max-w-8xl mx-auto px-6 lg:px-12 pt-10 pb-6">
        ...
        {!!category?.breadcrumb?.length && (
          <p className="font-ui text-[11px] tracking-[0.18em] text-mitti mt-4">{category.breadcrumb.join(' / ').toUpperCase()}</p>
        )}
        <h1 className="font-display text-4xl lg:text-5xl text-kohl mt-2">{category?.name || titleFromSlug(slug)}</h1>
        <p className="font-italic italic text-mitti mt-2">
          {loading ? 'Loading...' : `${pagination.total} ${pagination.total === 1 ? 'piece' : 'pieces'} · India's finest craft, curated by hand`}
        </p>
      ...
      <section className="max-w-8xl mx-auto px-6 lg:px-12 sticky top-20 z-30 bg-ivory border-b border-beige py-4 flex items-center justify-between gap-4">
        <button type="button" onClick={() => setMobileFiltersOpen(true)} className="lg:hidden btn-outline text-xs flex items-center gap-2" aria-haspopup="dialog" aria-expanded={mobileFiltersOpen}>
          <Filter className="w-4 h-4" /> FILTERS {activeFilters > 0 && `(${activeFilters})`}
        </button>
        <p className="hidden lg:block font-ui text-xs tracking-widest text-mitti">{activeFilters > 0 ? `${activeFilters} FILTER${activeFilters > 1 ? 'S' : ''} APPLIED` : 'ALL ITEMS'}</p>
        <div className="flex items-center gap-3">
          {activeFilters > 0 && <button type="button" onClick={clearAll} className="font-ui text-xs text-madder hover:underline">CLEAR ALL</button>}
          <div className="relative">
            <select value={filters.sort} onChange={(e) => setParam('sort', e.target.value)} className="appearance-none bg-beige px-4 py-2 pr-9 font-ui text-xs tracking-widest cursor-pointer" aria-label="Sort products">
      ...
      <section className="max-w-8xl mx-auto px-6 lg:px-12 py-8 grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-10">
        <aside className="hidden lg:block"><FilterPanel facets={facets} filters={filters} setParam={setParam} /></aside>

        {mobileFiltersOpen && (
          <div className="lg:hidden fixed inset-0 bg-kohl/60 z-50 flex items-end" onClick={() => setMobileFiltersOpen(false)} role="presentation">
            <div onClick={(e) => e.stopPropagation()} className="bg-ivory w-full max-h-[85vh] overflow-y-auto p-6" role="dialog" aria-modal="true" aria-labelledby="mobile-filter-title">
              <div className="flex items-center justify-between mb-6">
                <h2 id="mobile-filter-title" className="font-display text-xl text-kohl">Filters</h2>
                <button type="button" onClick={() => setMobileFiltersOpen(false)} aria-label="Close filters"><X className="w-5 h-5" aria-hidden="true" /></button>
              </div>
              <FilterPanel facets={facets} filters={filters} setParam={setParam} />
              <button type="button" onClick={() => setMobileFiltersOpen(false)} className="btn-primary w-full mt-6">APPLY FILTERS · {pagination.total}</button>
            </div>
          </div>
        )}
        ...
        <div>
          {loading ? (
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-6" aria-label="Loading products" aria-busy="true">
              {Array.from({ length: 6 }).map((_, index) => <div key={index} className="aspect-[3/4] bg-beige animate-pulse" aria-hidden="true" />)}
            </div>
          ) : products.length === 0 ? (
            <div className="py-20 text-center">
              ...
              {activeFilters > 0 && <button type="button" onClick={clearAll} className="btn-outline mt-6">CLEAR ALL FILTERS</button>}
            </div>
          ) : (
            <>
              ...
              {pagination.totalPages > 1 && (
                <nav className="mt-14 flex items-center justify-center gap-4" aria-label="Product pages">
                  <button type="button" disabled={!pagination.hasPrevPage || loading} onClick={() => setParam('page', String(Math.max(1, pagination.page - 1)))} className="btn-outline min-w-[120px] disabled:opacity-40 disabled:cursor-not-allowed">PREVIOUS</button>
                  <span className="font-ui text-xs tracking-widest text-mitti" aria-live="polite">PAGE {pagination.page} OF {pagination.totalPages}</span>
                  <button type="button" disabled={!pagination.hasNextPage || loading} onClick={() => setParam('page', String(Math.min(pagination.totalPages, pagination.page + 1)))} className="btn-outline min-w-[120px] disabled:opacity-40 disabled:cursor-not-allowed">NEXT</button>
                </nav>
              )}
            </>
          )}
...
function FilterPanel({ facets, filters, setParam }: { facets: FacetsState; filters: { craft: string; region: string; material: string; occasion: string; badge: string; minPrice: string; maxPrice: string; sort: string; q: string; page: number }; setParam: (key: string, value: string) => void }) {
    ...
    <div className="space-y-6 font-ui text-sm">
      ...
      <div>
        ...
        <div className="flex gap-2 items-center">
          <input type="number" min="0" placeholder={String(minR || 0)} value={filters.minPrice} onChange={(e) => setParam('minPrice', e.target.value)} className="w-20 p-2 bg-beige border border-mitti/20 text-xs" aria-label="Minimum price" />
          <span className="text-mitti" aria-hidden="true">-</span>
          <input type="number" min="0" placeholder={String(maxR || 50000)} value={filters.maxPrice} onChange={(e) => setParam('maxPrice', e.target.value)} className="w-20 p-2 bg-beige border border-mitti/20 text-xs" aria-label="Maximum price" />
        </div>
        {(minR || maxR) && <p className="font-ui text-[10px] text-mitti mt-2">Range: {formatINR(facets.priceRange.minPaise)} - {formatINR(facets.priceRange.maxPaise)}</p>}
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div className="space-y-1.5">
        {options.slice(0, 10).map(([name, count]) => (
          <button key={name} type="button" onClick={() => onChange(current === name ? '' : name)} aria-pressed={current === name} className={`flex items-center justify-between w-full text-left text-xs py-1 ${current === name ? 'text-madder font-medium' : 'text-kohl hover:text-madder'}`}>
            <span>{name}</span><span className="text-mitti">({count})</span>
          </button>
        ))}
      </div>
    </div>
