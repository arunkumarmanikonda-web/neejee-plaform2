...
function MirrorInner() {
  ...
  const [selectedVariantId, setSelectedVariantId] = useState<string>(initialVariant);
  ...
  useEffect(() => {
    ...
    const nextWithSelection = query.toString() ? `/ai/mirror?${query.toString()}` : '/ai/mirror';
    const loginUrl = `/login?next=${encodeURIComponent(nextWithSelection)}`;
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          ...
          if (response.ok) {
            ...
            if (data?.email) { setAuthChecking(false); return; }
          }
        } catch {}
    ...
    fetch('/api/products?limit=60')
      ...
      .then(data => {
        const list = (data.products || []).filter((product: any) => product.aiTryOnEligible);
        setEligible(list);
  ...
  const generate = async () => {
    ...
    setGenerating(true);
    setError('');
    setStubMessage('Submitting your portrait…');
    ...
    try {
      ...
      try { data = raw ? JSON.parse(raw) : {}; } catch {
        throw new Error('Could not reach the mirror. Please try again.');
      }
      if (!response.ok) {
      ...
      if (!data.pollUrl) throw new Error('Mirror returned no poll URL.');
      const pollUrl: string = data.pollUrl;
      ...
      while (Date.now() - start < maxWaitMs) {
        pollCount++;
        const statusResponse = await fetch(pollUrl, { credentials: 'include', cache: 'no-store' });
        const statusRaw = await statusResponse.text();
        let statusData: any = {};
        try { statusData = statusRaw ? JSON.parse(statusRaw) : {}; } catch {}
        if (!statusResponse.ok) throw new Error(statusData?.error || `Generation failed (status ${statusResponse.status})`);
        if (statusData.done && statusData.outputUrl) {
        ...
        const niceStatus = statusData.status === 'IN_QUEUE'
          ? `Waiting in the queue…${statusData.queuePosition !== undefined ? ` (position ${statusData.queuePosition})` : ''}`
          : statusData.status === 'IN_PROGRESS'
            ? `Mirroring you onto the piece…${statusData.lastLog ? ` · ${statusData.lastLog}` : ''}`
            : `Status: ${statusData.status || 'thinking…'}`;
        setStubMessage(`${niceStatus} (check ${pollCount})`);
        if (statusData.status) lastStatus = statusData.status;
        await new Promise(resolve => setTimeout(resolve, 3000));
      }
      throw new Error(`The mirror is taking longer than usual (last status: ${lastStatus}). Please try again.`);
    } catch (e: any) {
      setError(e.message);
    } finally {
      setGenerating(false);
  ...
  return (
    <>
      ...
      <section className="bg-kohl text-ivory py-16 px-6 text-center">
        <p className="text-xs tracking-[0.35em] text-banarasi mb-4">THE NEEJEE MIRROR</p>
        <h1 className="font-display text-5xl md:text-6xl">See how it may live on you.</h1>
        <p className="font-italic italic text-ivory/70 max-w-xl mx-auto mt-4">
          An imagined look, personally rendered. Preview the mood, not a promise of exact fit.
        </p>
      </section>

      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="flex items-center justify-center gap-2 mb-12 text-[10px] tracking-[0.25em]">
          {['UPLOAD', 'CHOOSE', 'MIRROR'].map((label, index) => {
            const current = ['upload', 'select', 'preview'].indexOf(step);
            const active = index <= current;
            return (
              <div key={label} className="flex items-center gap-2">
                <span className={`w-6 h-6 rounded-full flex items-center justify-center ${active ? 'bg-madder text-ivory' : 'bg-beige text-mitti'}`}>{index + 1}</span>
            ...
            <label className="block border-2 border-dashed border-mitti/40 hover:border-kohl bg-beige/30 cursor-pointer p-12 transition-colors">
              ...
              {uploading ? (
                ...
                <>
                  ...
                  <p className="text-[10px] tracking-wider text-mitti/70 mt-3">Best: front-facing, even light, full upper body in frame.</p>
                </>
              )}
            </label>
          ...
          <div>
            <div className="flex items-start gap-6 mb-8">
              ...
              <div className="flex-1">
                ...
                <p className="font-display text-lg text-kohl mt-1">Ready for the mirror.</p>
                <button onClick={reset} className="text-xs tracking-wider text-madder underline mt-2">Use a different portrait</button>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-4 border border-madder/30 bg-ivory p-3">
                  {selectedProduct.images?.[0] && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={selectedProduct.images[0]} alt={selectedProduct.name} className="w-20 h-24 object-cover" />
                  )}
                  <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {eligible.map(product => (
                      ...
                      >
                        {product.images?.[0] && (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={product.images[0]} alt={product.name} className="w-full aspect-[3/4] object-cover" />
                        )}
                        <div className="p-2">
            ...
            <label className="flex items-start gap-2.5 mt-6 cursor-pointer">
              ...
              <span className="text-xs text-kohl/85 leading-relaxed">
                I consent to NEEJEE processing my portrait to generate this private AI preview and understand that AI visualisation may differ from actual fit, drape, colour and scale.
              </span>
            </label>
            ...
            {generating && stubMessage && <p className="text-xs text-mitti italic mt-3 text-center">{stubMessage}</p>}
            {generating && <p className="text-xs text-mitti/60 mt-2 text-center">Generation can take a little while. Keep this tab open.</p>}
          </div>
        )}
        ...
        {step === 'preview' && (
          <div>
            ...
            <div className="text-center">
              <p className="font-display text-3xl text-kohl mb-2">There you are.</p>
              <p className="font-italic italic text-mitti text-sm mb-6">An imagined look, personally rendered.</p>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={outputUrl} alt="AI Mirror preview" className="max-w-md w-full mx-auto border border-mitti/30 mb-6" />
