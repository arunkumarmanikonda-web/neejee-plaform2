...
export default function MarketingStudioPage() {
  ...
  // v23.34.1 ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â multi-select format & style. Default to a sensible 3-format bundle.
  const [formats, setFormats] = useState<Format[]>(['ig_square', 'ig_story', 'fb_banner']);
  const [styles, setStyles] = useState<Style[]>(['editorial']);
  // v23.34.1 ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â branded overlay controls
  const [brandLogo, setBrandLogo] = useState(true);
  const [ctaText, setCtaText] = useState('SHOP NOW');
  const [badgesCsv, setBadgesCsv] = useState('FOUNDERÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¢S EDIT ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· HANDLOOM CERTIFIED');
    ...
    try {
      const r = await fetch('/api/admin/marketing-studio/generate', {
        ...
        body: JSON.stringify({
          ...
          productBadges: badgesCsv.split(/[,ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â·]/).map(s => s.trim()).filter(Boolean),
        }),
      });
  ...
  const saveToAssets = async (variantIdx: number) => {
    ...
    const alt = `Marketing Studio ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â ${productCtx} (${variant.style || ''})`;
    try {
    ...
    <div className="space-y-6 max-w-7xl">
      <div>
        ...
        <p className="text-mitti text-sm">
          AI-generated creatives & copy for Instagram, email, WhatsApp ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â pick what to feature, get 4 variants in seconds.
        </p>
      </div>
      ...
      <Section step="2" title="Pick the subject">
        ...
        {flow === 'generic' && (
          <textarea value={brief} onChange={e => setBrief(e.target.value)}
            rows={3}
            placeholder="e.g. We're celebrating 100 sarees sold from our Banarasi atelier ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â gratitude-forward, warm light, the loom in the background."
            className="w-full border border-mitti/30 px-3 py-2 font-ui text-sm bg-ivory" />
        )}
      </Section>

      {/* Step 3: Format & style ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â MULTI-SELECT */}
      <Section step="3" title="Format & style">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <p className="label text-banarasi mb-2">FORMATS ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· PICK ONE OR MANY</p>
                ...
                return (
                  <button key={f.v}
                    ...
                    <p className="font-display text-sm text-kohl flex items-center gap-1">
                      {selected && <span className="text-madder">ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã¢â‚¬Â¦ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“</span>} {f.l}
                    </p>
          ...
          <div>
            <p className="label text-banarasi mb-2">STYLES ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· PICK ONE OR MANY</p>
            <div className="grid grid-cols-2 gap-2">
              {STYLES.map(s => {
                ...
                return (
                  <button key={s.v}
                    ...
                    <p className="font-display text-sm text-kohl flex items-center gap-1">
                      {selected && <span className="text-madder">ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã¢â‚¬Â¦ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“</span>} {s.l}
                    </p>
        ...
        <div className="mt-5 grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-mitti/10 pt-4">
          <div className="md:col-span-3">
            ...
            <p className="text-[11px] text-mitti italic">Every creative is rendered with the NEEJEE wordmark, the badges you select, and a CTA button burned into the image ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â ready to post.</p>
          </div>
          ...
          <div>
            ...
            <input value={badgesCsv} onChange={e => setBadgesCsv(e.target.value)}
              placeholder="FOUNDERÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¢S EDIT, HANDLOOM CERTIFIED"
              className="w-full border border-mitti/30 px-2 py-1 font-ui text-xs bg-ivory" />
          </div>
        </div>

        <div className="mt-4">
          <p className="label text-banarasi mb-1">TONE OVERRIDE <span className="text-mitti font-normal">ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â optional</span></p>
      ...
      <div className="sticky bottom-4 bg-ivory border border-mitti/20 p-4 rounded shadow-lg flex items-center justify-between gap-3 flex-wrap">
        <p className="text-mitti text-sm">
          {ready
            ? `${formats.length * styles.length} creative${formats.length * styles.length === 1 ? '' : 's'} (${formats.length} format ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ${styles.length} style) + copy in ~60-90s`
        ...
        <button onClick={generate} disabled={generating || !ready}
          ...
          {generating
            ? <><Loader2 className="w-3 h-3 animate-spin" /> GENERATINGÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦</>
            ...
            {result.diagnostics?.mode && (
              <span className={`text-[10px] font-ui tracking-widest px-2 py-1 ${
                ...
                title={result.diagnostics.mode === 'image-edit'
                  ...
                  : 'No product photo found ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â generated from text only'}>
                {result.diagnostics.mode === 'image-edit'
                  ? `ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã¢â‚¬Â¦ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“ PRODUCT-LOCKED (${result.diagnostics.referenceImageCount} ref${result.diagnostics.referenceImageCount === 1 ? '' : 's'})`
                  : 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â  TEXT-ONLY (no product photo)'}
              </span>
            )}
          </div>

          {/* Responsive grid ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â cards keep their own aspect ratio per format */}
            ...
            {result.variants.map((v: any, i: number) => {
              ...
              <div key={i} className={`relative group border border-mitti/20 rounded overflow-hidden ${cls}`}>
                ...
                <span className="absolute top-2 left-2 bg-kohl/80 text-ivory text-[10px] tracking-widest px-2 py-0.5 rounded">
                  {fLabel.toUpperCase()} ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· {String(v.style || '').toUpperCase()}
                </span>
              </div>
              );
            })}
  ...
  return (
    <>
      <input value={search} onChange={e => setSearch(e.target.value)}
        placeholder="Search products by name or SKUÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦"
    ...
    <div>
      ...
      {tab === 'products' ? (
        ...
        <select value={categoryId} onChange={e => onCategoryChange(e.target.value)}
          ...
          <option value="">Pick a categoryÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦</option>
...
function OfferPicker({ coupons, couponId, onCouponChange, discountText, onDiscountTextChange }: any) {
  return (
    <div className="space-y-4">
      <div>
        <p className="label text-banarasi mb-1">PICK A COUPON <span className="text-mitti font-normal">ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â or describe the offer below</span></p>
        <select value={couponId} onChange={e => onCouponChange(e.target.value)}
          className="w-full border border-mitti/30 px-3 py-2 text-sm bg-ivory">
          <option value="">No specific coupon</option>
          {coupons.map((c: any) => (
            <option key={c.id} value={c.id}>{c.code} {c.discountType === 'PERCENT' ? `(${c.discountValue}% off)` : `(ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¹${c.discountValue / 100} off)`}</option>
          ))}
        </select>
      </div>
      <div>
        ...
        <input value={discountText} onChange={e => onDiscountTextChange(e.target.value)}
          placeholder="e.g. 20% off this weekend, free shipping above ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¹2,500"
          className="w-full border border-mitti/30 px-3 py-2 text-sm bg-ivory" />
      </div>
    </div>
  );
