...
export const SELLER_AGREEMENT_TEMPLATE_VERSION = 'NIAR-2026-07-22-v1';
...
function extractSellerAddress(summary: unknown) {
...
export const SELLER_AGREEMENT_CLAUSES = [
  {
    id: '1',
...
export async function buildSellerAgreementMaster(sellerId: string) {
  ...
  const company = {
    legalName: safeString(entity?.legalName, 'Oye Imagine Private Limited').trim(),
    brandName: safeString(entity?.brandName, 'NEEJEE').trim(),
    ...
    authorisedSignatory: safeString(entity?.authorisedSignatory, 'Nidhi Chauhan'),
  ...
  const commercialTerms = {
    commissionPct: typeof seller.commissionPct === 'number' ? seller.commissionPct : 20,
    qualityScore: typeof seller.qualityScore === 'number' ? seller.qualityScore : 0,
    payoutCycle: safeString(seller.payoutCycle, 'MONTHLY') || 'MONTHLY',
    isNeejeeSelect: !!seller.isNeejeeSelect,
    yearsOfPractice: seller.yearsOfPractice ?? null,
    cluster: seller.cluster ?? null,
  };

  const annexure = [
    { label: 'Seller legal name / business name', value: sellerBlock.businessName || '-' },
    { label: 'Registered / principal address', value: sellerBlock.address || '-' },
    ...
    { label: 'CIN', value: sellerBlock.cinNumber || '-' },
    { label: 'MSME / Udyam', value: sellerBlock.msmeNumber || '-' },
    { label: 'Craft / category', value: compact([sellerBlock.craft, sellerBlock.region]).join(' • ') || '—' },
    { label: 'Commission %', value: `${commercialTerms.commissionPct}%` },
    { label: 'Payout cycle', value: commercialTerms.payoutCycle },
    { label: 'Neejee Select', value: commercialTerms.isNeejeeSelect ? 'Yes' : 'No' },
    { label: 'Quality score', value: String(commercialTerms.qualityScore) },
    { label: 'Years of practice', value: commercialTerms.yearsOfPractice != null ? String(commercialTerms.yearsOfPractice) : '-' },
    { label: 'Cluster', value: safeString(commercialTerms.cluster, '—') },
    { label: 'Bank', value: compact([sellerBlock.bankName, sellerBlock.ifsc, sellerBlock.bankAccountMasked]).join(' • ') || '—' },
  ];

  return {
    generatedAt: new Date().toISOString(),
    templateVersion: SELLER_AGREEMENT_TEMPLATE_VERSION,
    ...
    seller: sellerBlock,
    commercialTerms,
    clauses: SELLER_AGREEMENT_CLAUSES.map((clause) => ({ ...clause, paragraphs: [...clause.paragraphs] })),
    annexure,
    execution: {
      ...
      placeOfExecution: companyAddress || 'Noida, Uttar Pradesh, India',
    },
  };
}
