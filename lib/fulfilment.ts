...
/**
 * A short status line for PDPs and cards. Empty string means there is no
 * exceptional fulfilment state. Keeping the helper consistently string-typed
 * makes it safe to render or transform after a truthy check in React.
 */
export function fulfilmentStatusLine(p: FulfilmentProduct): string {
  if (isPreorder(p)) {
