// lib/email/templates/recovery-t24h.ts
// T+24h — Story-led, 10% discount from karigars.

import { brandHeader, brandFooter, renderItems, discountPanel, RecoveryEmailVars } from './recovery-shared';

export interface T24hVars extends RecoveryEmailVars {
  discountCode: string;
  discountPercent: number;
  validHours: number;
}

export function recoveryT24hEmail(vars: T24hVars): { subject: string; html: string } {
  const name = vars.customerName?.split(' ')[0] || 'friend';
  const ai = vars.aiCopy;

  const subject = ai?.subject || `A small gift from our karigars`;
  const lede = ai?.lede || `Dear ${name}, the artisans behind your selections would like to send you a small gesture — ${vars.discountPercent}% off, with code ${vars.discountCode}.`;
  const body = ai?.body || `Each piece in your trunk took days of slow work — threads counted by hand, motifs drawn from memory.\n\nIf the price was holding you back, please accept this with our thanks. The code rests in your trunk until tomorrow evening.`;
  const signoff = ai?.signoff || `With warmth from the loom,\nThe NEEJEE atelier`;
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;background:#FFFEFB;">
        ...
        <tr><td style="padding:40px 36px 24px 36px;font-family:Georgia,serif;color:#1A1613;">
          ...
          <div style="font-size:13px;letter-spacing:0.25em;color:#776657;text-transform:uppercase;margin-bottom:18px;">A note from the karigars</div>
